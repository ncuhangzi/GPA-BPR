...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print('len of metapath_list:',len(self.metapath_list))
        ...
        for i in self.test_neg_ratings_list[idx]:
            user_input[k] = u
            item_input[k] = i
            # print('len of metapath_list2:',len(self.metapath_list))
            for metapath_idx in range(len(self.metapath_list)): #metapath list中有沒有包含這兩個節點的路徑 有的話取得其特徵
                ...
                if (u, i) in self.metapath_list[metapath_idx][1]:
                    ...
                    for p_i in range(len(self.metapath_list[metapath_idx][1][(u, i)])):
                        for p_j in range(len(self.metapath_list[metapath_idx][1][(u, i)][p_i])):
                            ...
                            #給予每個user和item其feature
                            metapath_input_list[metapath_idx][k][p_i][p_j] = self.features[node_type][node_id]#加了str()

            k += 1

        # positive item ids  查找該idx的embedding
        for i in self.test_ratings_list[idx][1:]:
            user_input[k] = u
            item_input[k] = i
            # print('len of metapath_list3:',len(self.metapath_list))
            ...
            if (u, i) in metapath[1]:
                for p_i in range(len(metapath[1][(u, i)])):
                    for p_j in range(len(metapath[1][(u, i)][p_i])):
                        ...
                        node_id = int(node_id)
                        node_type = self.id2type[type_id]
                        metapath_input[counter][p_i][p_j] = self.features[node_type][node_id] #加了str()
                ...
                if (u, j) in self.metapath_list[list_index][1]:
                    for p_i in range(len(self.metapath_list[list_index][1][(u, j)])):
                        for p_j in range(len(self.metapath_list[list_index][1][(u, j)][p_i])):
                            ...
                            node_id = int(node_id)
                            node_type = self.id2type[type_id]
                            metapath_input_list[list_index][counter][p_i][p_j] = self.features[node_type][node_id] #加了str()
            ...
            with open(feature_file_path) as input:
                ...
                for line in input.read().splitlines():
                    ...
                    if self.feature_size == -1:
                        self.feature_size = len(arr) - 2
                    else:
                        assert (self.feature_size == (len(arr) - 2))

                self.node_sizes[feature_type] = count
            print(feature_type, self.node_sizes[feature_type])

        for feature_type, feature_file_path in feature_file_dict.items(): #u -> t -> b
            if feature_type == "t":
                self.features[feature_type] = np.zeros((self.node_sizes[feature_type] + 43086, self.feature_size),
                                                    dtype=np.float32)
            else:
            ...
            with open(feature_file_path) as input:
                for line in input.readlines():
                    ...
                    for j in range(len(arr[2:])):
                        ...
                        self.features[feature_type][node_id][j] = float(arr[j + 2])
            ...
            with open(metapath_file) as input:
                for line in input.read().splitlines():
                    ...
                    for node in tmp:
                        ...
                        node_list.append([self.type2id[node[0]], index]) #檢索條件不一樣 需要調整
